...
class BaseModel(db.Model):
    ...
    update_time = db.Column(db.DateTime, index=True, default=db.func.now(), onupdate=db.func.now())
...
class User(UserMixin, BaseModel):
    ...
    introduce_user_code = db.Column(db.String(64))

    withdraw = db.Column(db.Integer, default=0)
    ...
    @staticmethod
    def insert_admin():
        user = User.create(email='shadow.ying3@gmail.com',
                           qq='835751617',
                           phone='16608003241',
                           wechat='unknown',
                           introduce_user_code=str(uuid.uuid1()),
                           password='xjv123..')
        user.user_code = '8e23ca0a-061f-11e9-aaf1-0800273a1782'
        db.session.add(user)
        db.session.commit()
...
class ArticleTypeSetting(BaseModel):
...
class Order(BaseModel):
    __tablename__ = 'order'
    id = db.Column(db.Integer, primary_key=True)
    user_code = db.Column(db.String(64))
    introduce_user_code = db.Column(db.String(64))
    ...
    @staticmethod
    def create(user_code=user_code, introduce_user_code=introduce_user_code, email=email, number=number):
        o = Order(user_code=user_code,
